...
> 💡 **Code Design Note**: Notice how the test defines the API before implementation exists. You're deciding: "_I want a class `PasswordValidator` with a `validate` method that returns an object with `isValid` and `errors`._" This is how TDD drives design from the user's perspective.
...
> 💡 **Living Test List**: The test list isn't carved in stone. Add tests as you discover or think of behavior that needs verification, like configurability here.
>
