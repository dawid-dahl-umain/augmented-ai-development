## Level 5: See TDD Catch AI Mistakes
...
![See TDD Catch AI Mistakes](../../../assets/onboarding/5.webp)
...
Let's see a real scenario where AI tries to be "helpful" but TDD discipline saves you from shipping bugs and over-engineering. This example demonstrates **Way 1: Business Behavior** from Level 3.
...
**AI without constraints**: _"Here's a comprehensive and production-ready solution_ ✅"
...
- ❌ Spec asked for 2 validation rules (length + number)
- ❌ AI added 4 extra unauthorized rules (uppercase, special char, max length, common password)
- ❌ Untested edge cases shipped to production
- ❌ Users complain their valid passwords are rejected
- ❌ Business never approved these rules
...
> 💡 **How AAID actually works**: As described in [Level 2](./2.md), the AI reads the rules file and follows TDD discipline. After each phase, it stops and asks if you want to continue. You simply say for example "yes", "go" or provide guidance.
>
> If the AI ignore the rules file—it unfortunately happens sometimes—you can use commands like `/refactor-&-stop` to get it back on track.
...
> A **test list** is your initial idea of what you will test. It's a "living" list, meaning you can add, remove or change tests in the list as you go along, if needed. Important thing is that the test list only contain the test names, not the test implementations; those will be built in the TDD cycle itself.

Un-skip and implement these one at a time through RED → GREEN → REFACTOR cycles. Watch how the tests drive minimal, focused code.

---

<a id="cycle-1"></a>

## 🔄 Cycle 1: First Test (Invalid Password)
...
> 💡 **Code Design Note**: Notice how the test defines the API before implementation exists. You and the AI agent is deciding: "_I want a class `PasswordValidator` with a `validate` method that returns an object with `isValid` and `errors`._" This is how TDD drives design from the user's perspective.
...
### 🟢 GREEN: Make It Pass (The Simplest Way Possible)
...
describe("PasswordValidator", () => {
  ✅ it("should reject passwords that are too short")
  it.skip("should respect custom minimum length")  // ← Added
  it.skip("should accept passwords at minimum length")
  it.skip("should reject passwords without numbers")
})
```

> 💡 **Living Test List**: As mentioned before, your test list isn't carved in stone. Here, we added a test after discovering new behavior that needs verification, such as configurability.
>
...
## 🔄 Cycle 2: Test Custom Minimum Length
...
**Run tests**: ❌ Fails (`PasswordValidator` constructor doesn't exist yet)
...
> 💡 **Test-Driven Design**: See how the test is the first user of your code? Again, RED is where you design the API; how it _should_ be used before implementing how it works.

**Run tests**: ❌ Still fails (constructor doesn't exist yet)
...
### 🟢 GREEN: Add Constructor Parameter
...
> 💡 **Note**: For brevity, we'll omit unchanged type definitions in subsequent code blocks.

**Run tests**: ✅ Both tests pass (first uses default 8, second uses custom 10)
...
**Run tests**: ❌ Fails (hardcoded response always returns invalid)
...
### 🟢 GREEN: Now Real Logic Is Needed
...
**Run tests**: ✅ All three tests pass

**The "aha!" moment**: The test **forced** actual validation logic. You couldn't fake it anymore: two tests demanded real behavior. This is how TDD keeps code minimal and focused, instead of over-engineered.
...
**💡 The TDD boundary at work**: You've completed Scenario 1 (length validation). Notice what's **NOT** in the code:

- ❌ No uppercase requirement (wasn't in spec)
- ❌ No special character requirement (wasn't in spec)
- ❌ No "common password" check (wasn't in spec)
- ❌ No max length limit (wasn't in spec)
...
describe("PasswordValidator", () => {
  ...
  ✅ it("should respect custom minimum length")
  ✅ it("should accept passwords at minimum length")
  it.skip("should reject passwords without numbers")  // ← Next
})
```
...
### 🧼 REFACTOR: Developer Guides the Abstraction
...
> 💡 It is perfectly normal that you and the AI don't always agree. Remember: **this is not vibe coding → you are in charge**!
>
...
> 💡 Notice how you are fully in charge, while the AI is generating the code? This is the beauty of the `AAID` **Developer Mindset**:
>
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------------------- |
| **Spec adherence**            | ❌ Spec asked for 2 rules, AI added 4 unauthorized | ✅ Implemented only what's in spec (length + number) |
| **Testing coverage**          | ❌ Untested code shipped to production             | ✅ Every line tested before shipping                 |
| **User impact**               | ❌ Valid passwords like "password123" rejected     | ✅ All spec-compliant passwords work                 |
| **Unauthorized features**     | ❌ Business never approved these rules             | ✅ Only implements spec-approved requirements        |
| **Code quality & complexity** | ❌ 40+ lines with unnecessary features             | ✅ ~25 lines of clean, easily extendable code        |

**This demonstrates the Three Laws of TDD:**

1. **Write no production code without a failing test** → Prevented unapproved features
2. **Write only enough test code to fail** → Kept tests focused on spec scenarios
3. **Write only enough production code to pass** → Stopped over-engineering

Test defines the boundary. No test? No code.

---

**🛑 Checkpoint**: See how TDD discipline guards against AI over-engineering?

- **"Yes, this makes sense!"** → Continue to [Level 6](./6.md) for next steps
- **"I want to try this"** → Jump to [Get Started](./get-started.md) for setup
- **"Show me the workflow again"** → Review [Level 4](./4.md)

The key point: TDD discipline focuses AI's capabilities in the right direction. Your reviews ensure it stays on track.

---

✅ **Progress: 75%** | Next: Level 6 (5 minutes) - Will This Work for Me?
