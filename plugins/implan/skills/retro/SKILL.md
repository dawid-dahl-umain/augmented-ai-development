...
The default location is a single `RETRO.md` at the root of the plan directory. To find that directory, look under `ai-plans/` for folders containing a `MAIN.md`; that file is what makes a folder a plan directory. Standalone spike folders also live under `ai-plans/` and are not plan directories.
